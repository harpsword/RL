...
from model import Policy2013
model_storage_path = '/home2/yyl/model/programRL/'
...
def get_reward(model, env):
    ...
    if ep_max_step is None:
        ...
        for step in range(ep_max_step):
            action = model.act(ProcessU.to_torch_tensor())
...
def test(model, pool, env, test_times):
    ...
    return rewards, timesteps, timesteps_count
...
def main(model, gamename, ncpu):
    ...
    env = gym.make(gamename)
    test_model = Policy2013(env.action_space.n)

    test_times = 100
    
    test_model.load_state_dict(torch.load(os.path.join(model_storage_path, model)))

    test_rewards, timesteps, _ = test(test_model, pool, env, test_times)

    print("test results:", np.array(test_rewards).mean())
    print(str(test_rewards))

    print("test timesteps results:", np.array(timesteps).mean())
    print(str(timesteps))
